...
//!
//! Environment variables are resolved in [`crate::config`] (not via clap) so
//! the deprecated legacy `WHYBIG_DATA_DIR` can fall back behind
//! `DISKDRIFT_DATA_DIR` with a single ordering rule.
...
///
/// Track disk growth over time and find what got big.
#[derive(Debug, Parser, Clone)]
#[command(
    name = "diskdrift",
    version,
    about = "Track disk growth over time and find what got big.",
    long_about = None,
...
pub struct Cli {
    ...
    /// (default: platform app-data dir; env DISKDRIFT_DATA_DIR).
...
pub enum Command {
    /// Create the data directory and database (idempotent).
    Init,
    ...
    /// Preview and apply DiskDrift's snapshot retention policy.
    ///
